...
from proot_distro.message import crit_error, log_error, warn
...
class _FlockBase:
    ...
    def acquire(self) -> bool:
        ...
        Robustness note: if the lock directory cannot be created or the
        lock file cannot be opened, we *refuse* to proceed (return False
        + log_error) rather than silently proceeding unlocked — the old
        behavior risked concurrent modification of containers on broken
        filesystems. The only exception is flock not being supported by
        the filesystem (e.g. some FUSE mounts), where we still proceed
        but with an explicit warning so the user understands locking is
        best-effort.
        """
        ...
        try:
            ...
            log_error(f"cannot create lock directory {lock_dir}: {exc}")
            return False

        # Open with "a+" (read+append, no truncate) so that a failed
        # flock attempt doesn't clobber the PID/command recorded by the
        # current lock holder. We truncate explicitly only after
        # successfully acquiring the lock.
        try:
            fd = open(self._lock_path, "a+")
        except OSError as exc:
            log_error(f"cannot open lock file {self._lock_path}: {exc}")
            return False
        ...
        try:
            ...
            # flock not supported on this filesystem — proceed unlocked
            # but warn so the user understands locking is best-effort.
            warn(f"flock not supported on {self._lock_path}, proceeding without lock")
            return True

        # Record PID + command in the file for diagnostic purposes.
        # Truncate the file *after* acquiring the lock so we don't
        # destroy the previous holder's diagnostic info on a failed
        # attempt.
        try:
            fd.seek(0)
            fd.truncate()
    ...
    def __enter__(self):
        if not self.acquire():
            ...
            crit_error(_("{label} '{name}' is busy{hint}.", label = self._label, name = self._display, hint = hint))
            sys.exit(1)
        return self
