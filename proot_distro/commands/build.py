...
def command_build(args):
    ...
    if dockerfile_path is not None and not dockerfile_path:
        crit_error(_("Dockerfile path cannot be empty."))
        sys.exit(1)

    for out_file in outputs:
        if not out_file:
            crit_error(_("output file path cannot be empty."))
            sys.exit(1)

    if install_as is not None and not install_as:
        crit_error(_("--install-as value cannot be empty."))
        sys.exit(1)
    ...
    if not os.path.isdir(build_dir):
        crit_error(_("build context '{build_dir}' is not a directory.", build_dir = build_dir))
        sys.exit(1)

    if dockerfile != "-" and not os.path.isfile(dockerfile):
        crit_error(_("required file '{dockerfile}' does not exist.", dockerfile = dockerfile))
        sys.exit(1)
    ...
    try:
        ...
        crit_error(_("cannot read Dockerfile: {exc}", exc = exc))
        sys.exit(1)

    try:
        _directives, instructions = parse_dockerfile(text)
    except DockerfileSyntaxError as exc:
        crit_error(_("syntax error in Dockerfile: {exc}", exc = exc))
        sys.exit(1)

    if not instructions:
        crit_error(_("no instructions in Dockerfile."))
        sys.exit(1)
    ...
    if override_arch:
        ...
        if target_arch is None:
            crit_error(_("unknown architecture '{override_arch}'.", override_arch = override_arch))
            sys.exit(1)
    else:
    ...
    with ExitStack() as lock_stack:
        ...
        try:
            try:
                ...
                log_error(_("Build failed: {exc}", exc = exc))
                sys.exit(1)
            ...
            for t in tags:
                try:
                    ...
                    log_error(_("Cannot write manifest cache for '{t}': {exc}", t = t, exc = exc))
                    sys.exit(1)
            ...
            for out_file in outputs:
                ...
                try:
                    if not quiet:
                        log_info(_("Writing OCI archive to '{out_abs}'...", out_abs = out_abs))
                    write_oci_archive(out_abs, manifest, image_config, primary_tag)
                except (OSError, RuntimeError) as exc:
                    log_error(_("Cannot write '{out_file}': {exc}", out_file = out_file, exc = exc))
                    sys.exit(1)

            # Build summary.
            if not quiet:
                total_size = sum(l["size"] for l in final_stage.layers)
                log_info(_("Build complete."))
                msg()
                msg(f"{C['CYAN']}Tag(s): "
...
def _install_as_container(install_name, image_ref, target_arch, quiet):
    ...
    if not quiet:
        log_info(_("Installing built image as '{install_name}'...", install_name = install_name))
