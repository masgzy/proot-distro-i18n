...
def command_rename(args) -> None:
    ...
    if orig == new:
        crit_error(_("original and new names must differ."))
        sys.exit(1)
    ...
    if not os.path.isdir(orig_rootfs):
        crit_error(_("container '{orig}' is not installed.", orig = orig))
        sys.exit(1)

    if os.path.isdir(new_dir):
        crit_error(_("container '{new}' already exists.", new = new))
        sys.exit(1)
...
def _do_rename(orig, new, orig_dir, new_dir, new_rootfs, orig_rootfs):
    log_info(_("Renaming '{orig}' to '{new}'...", orig = orig, new = new))
    try:
        os.rename(orig_dir, new_dir)
    except OSError as exc:
        log_error(_("Failed to rename container: {exc}", exc = exc))
        sys.exit(1)

    rewrite_l2s_targets(new_rootfs, orig_rootfs)
    log_info(_("Finished renaming the container."))
