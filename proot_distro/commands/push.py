...
def command_push(args):
    ...
    if not image_ref:
        crit_error(_("image reference is not specified (e.g. 'myrepo/myapp:1.0')."))
        sys.exit(1)
    ...
    # network connection.
    manifest, _cfg, _layers = load_manifest_cache(image_ref, target_arch)
    if manifest is None:
        crit_error(
    ...
    registry, _repo, _tag = parse_image_ref(image_ref)
    ...
    try:
        ...
        if sys.stderr.isatty():
            sys.stderr.write("\r\033[K")
            sys.stderr.flush()
        log_error(_("Network error: {exc}", exc = exc))
        sys.exit(1)
