...
def _ensure_parent_writable(path: str) -> None:
    ...
    try:
        ...
        log_error(_("Cannot make '{parent}' writable: {exc}", parent = parent, exc = exc))
        sys.exit(1)
...
def _ensure_writable(path: str) -> None:
    ...
    try:
        ...
        log_error(_("Cannot set write permission on '{path}': {exc}", path = path, exc = exc))
        sys.exit(1)
...
def _sync_dir(dst_path: str, src_st: os.stat_result) -> bool:
    ...
    try:
        ...
        try:
            ...
            log_error(_("Cannot create directory '{dst_path}': {exc}", dst_path = dst_path, exc = exc))
            sys.exit(1)
    except OSError as exc:
        log_error(_("Cannot create directory '{dst_path}': {exc}", dst_path = dst_path, exc = exc))
        sys.exit(1)
...
def _sync_symlink(src_path: str, dst_path: str) -> bool:
    ...
    if os.path.lexists(dst_path):
        ...
        try:
            ...
            try:
                ...
                log_error(_("Cannot remove existing path '{dst_path}': {exc}", dst_path = dst_path, exc = exc))
                sys.exit(1)
        except OSError as exc:
            log_error(_("Cannot remove existing path '{dst_path}': {exc}", dst_path = dst_path, exc = exc))
            sys.exit(1)
    ...
    try:
        ...
        try:
            ...
            log_error(_("Cannot create symlink '{dst_path}': {exc}", dst_path = dst_path, exc = exc))
            sys.exit(1)
    except OSError as exc:
        log_error(_("Cannot create symlink '{dst_path}': {exc}", dst_path = dst_path, exc = exc))
        sys.exit(1)
...
def _sync_file(src_path: str, src_st: os.stat_result, dst_path: str) -> None:
    ...
    Writes to a unique sibling temp file (mkstemp) then atomically
    renames so a partial write never leaves dst_path in a corrupt
    state. The unique name avoids concurrent sync operations clobbering
    each other's temp file.
    """
    import tempfile
    dest_dir = os.path.dirname(dst_path) or "."
    ...
    try:
        ...
        try:
            ...
            try:
                ...
                pass
            log_error(_("Cannot write to '{dst_path}': {exc}", dst_path = dst_path, exc = exc))
            sys.exit(1)
    except OSError as exc:
        try:
            os.unlink(tmp)
    ...
    try:
        ...
        try:
            ...
            try:
                ...
                pass
            log_error(_("Cannot replace '{dst_path}': {exc}", dst_path = dst_path, exc = exc))
            sys.exit(1)
    except OSError as exc:
        try:
            os.unlink(tmp)
...
def _unlink_robust(path: str) -> None:
    ...
    try:
        ...
        try:
            ...
            log_error(_("Cannot delete '{path}': {exc}", path = path, exc = exc))
            sys.exit(1)
    except OSError as exc:
        log_error(_("Cannot delete '{path}': {exc}", path = path, exc = exc))
        sys.exit(1)
...
def _rmtree_robust(path: str) -> None:
    ...
    try:
        ...
        try:
            ...
            log_error(_("Cannot remove '{path}': {exc}", path = path, exc = exc))
            sys.exit(1)
    except OSError as exc:
        log_error(_("Cannot remove '{path}': {exc}", path = path, exc = exc))
        sys.exit(1)
...
def _do_sync(src, dest, verbose, use_checksum, delete):
    ...
    try:
        ...
        crit_error(_("source path '{src}' does not exist.", src = src))
        sys.exit(1)

    src_is_dir = stat.S_ISDIR(src_st.st_mode)

    if src_is_dir and not os.access(src_path, os.R_OK | os.X_OK):
        crit_error(_("source directory '{src}' is not readable.", src = src))
        sys.exit(1)
    ...
    log_info(_("Synchronizing files..."))
    log_info(_("Source: '{src_path}'", src_path = src_path))
    log_info(_("Destination: '{dest_path}'", dest_path = dest_path))
    ...
    if src_is_dir:
        try:
            ...
            try:
                ...
                log_error(_("Cannot create destination '{dest_path}': {exc}", dest_path = dest_path, exc = exc))
                sys.exit(1)
        except OSError as exc:
            log_error(_("Cannot create destination '{dest_path}': {exc}", dest_path = dest_path, exc = exc))
            sys.exit(1)
    ...
    try:
        for abs_path, rel_path in entries:
            try:
                ...
                log_error(_("Warning: cannot stat '{abs_path}': {exc}", abs_path = abs_path, exc = exc))
                done += 1
                _show_progress()
                continue
    ...
    if delete and src_is_dir:
        ...
        for del_n, (path, is_tree) in enumerate(extras, start=1):
            if verbose:
                log_info(_("({del_n}/{del_total}) Delete: {path}", del_n = del_n, del_total = del_total, path = path))
            if is_tree:
                _rmtree_robust(path)
            else:
                _unlink_robust(path)

    log_info(_("Finished synchronizing."))
