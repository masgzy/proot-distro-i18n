            ...
            if entry.is_dir(follow_symlinks=False):
                if verbose:
                    for dirpath, _dirs, filenames in os.walk(entry.path):
                        for fname in filenames:
                            log_info(
                                _("Removing: '{path}'", path = os.path.join(dirpath, fname))
                            )
                shutil.rmtree(entry.path)
            else:
                if verbose:
                    log_info(_("Removing: '{path}'", path = entry.path))
                os.remove(entry.path)
        except OSError as exc:
            log_error(_("Cannot remove '{path}': {exc}", path = entry.path, exc = exc))

    log_info(_("Reclaimed {size} of disk space.", size = fmt_size(total)))
