...
def read_passwd_entry(rootfs: str, user: str) -> dict | None:
    """Return all fields for *user* from /etc/passwd in one read.

    Replaces the old pattern of calling read_passwd_field() 4-5 times
    (one per field), each of which re-opened and re-scanned the file.
    Returns a dict with keys: name, password, uid, gid, gecos, home,
    shell; or None if the user is not found.

    Fields beyond the available ones default to "" so callers can use
    .get() without IndexError.
    """
    try:
        passwd = resolve_rootfs_path(rootfs, "/etc/passwd")
    except OSError:
