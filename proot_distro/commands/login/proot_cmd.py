...
def _add_custom_binds(args, custom_binds):
    ...
    for bnd in custom_binds:
        if not bnd:
            crit_error(_("bind specification cannot be empty."))
            sys.exit(1)
        if ":" in bnd:
            src, dst = bnd.split(":", 1)
        else:
            src, dst = bnd, None
        if not src:
            crit_error(_("bind source path cannot be empty in '--bind={bnd}'.", bnd = bnd))
            sys.exit(1)
        src = os.path.abspath(src)
        if dst is not None and not os.path.isabs(dst):
            crit_error(
                _("binding destination must be an absolute path, got '{dst}'.", dst = dst)
            )
            sys.exit(1)
