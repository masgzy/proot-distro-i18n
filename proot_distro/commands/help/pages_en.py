...
HELP_PAGES = {
    ...
    "mirror": {
        ...
        "aliases": ("mir", "mr"),
        "summary": (
            "Manage built-in Docker registry mirrors. The original "
            "proot-distro hardcoded a single registry URL in "
            "transport.py with no way to switch. This subcommand makes "
            "mirror switching a first-class operation: visible, "
            "testable, switchable."
            "\n\n"
            "Ships 17 built-in mirrors (DaoCloud, 1Panel, 1ms.run, "
            "Xuanyuan x2, Tencent Cloud, Docker Hub official, "
            "DockerProxy x2, Registry Cyou, Jiaxin, HubFast, Unsee, "
            "Cnxiaobai, gh-proxy x3), calibrated via web search to "
            "2026-07-08."
            "\n\n"
            "Users can add custom mirrors in ~/.config/proot-distro/"
            "mirrors.json; entries with the same id override built-ins."
            "\n\n"
            "Priority: PD_REGISTRY_URL env var > user config file > "
            "built-in default (DaoCloud)."
        ),
        "options": [
            ("-h, --help", "Show this help."),
            ("-t, --timeout [SECONDS]",
             "Probe timeout for `test` (default: 6 seconds)."),
            ("--allow-insecure",
             "Allow HTTPS to skip certificate verification. Only for "
             "self-signed private mirrors."),
            ("--use-best",
             "After `test`, auto-switch to the fastest reachable "
             "mirror without prompting. Script-friendly."),
            ("--no-use",
             "After `test`, do not prompt or switch."),
            ("--json",
             "`test` output as JSON (script-friendly; implies --no-use)."),
            ("--reachable-only",
             "`ls`: show only reachable mirrors (probes them first, "
             "may be slow)."),
            ("-q, --quiet",
             "Quiet mode. `ls` prints only URLs, one per line."),
        ],
        "examples": [
        ...
        "footer": [
            {
                "title": "SUB-ACTIONS",
                "intro": (
                    "ls / list: list all mirrors (built-in + "
                    "user-defined) + the currently effective one.\n"
                    "test [ID|URL]: probe reachability. Without args, "
                    "probes all concurrently and prompts Y/n to switch "
                    "to the fastest.\n"
                    "use / set ID|URL: pick a mirror (by id, name, or URL).\n"
                    "unset / clear / off: clear the current selection, "
                    "fall back to default.\n"
                    "show: print only the currently effective URL "
                    "(script-friendly, e.g. $(pd mirror show))."
                ),
            },
            {
                "title": "PD_REGISTRY_URL ENVIRONMENT VARIABLE",
                "intro": (
                    "PD_REGISTRY_URL has the highest priority and "
                    "overrides all config. When set: `mirror ls` "
                    "annotates 'env override'; `mirror use`/`unset` "
                    "warn that config would be ignored; `mirror show` "
                    "outputs the env URL. Run `unset PD_REGISTRY_URL` "
            ...
            },
            {
                ...
                "intro": (
                    "Follows Docker Registry HTTP API V2: issues "
                    "GET /v2/. 2xx/4xx both count as reachable "
                    "(401/403 means auth required but service is up); "
                    "only network errors or timeouts count as failure. "
                    "Concurrent probing uses ThreadPoolExecutor(max_workers=8)."
                ),
            },
            {
                "title": "CONFIG FILE",
                "intro": (
                    "Desktop Linux: ~/.config/proot-distro/mirror.json "
                    "(or $XDG_CONFIG_HOME/proot-distro/mirror.json).\n"
                    "Termux/Android: $PREFIX/etc/proot-distro/mirror.json.\n"
                    "Writes use atomic-replace (tmp + os.replace) so an "
                    "interrupt never leaves a half-written config."
                ),
            },
        ],
    },
...
TOP_COMMANDS = [
    ...
    ("mirror", "Manage Docker registry mirrors (ls/test/use/unset/show)."),
]
