...
# Section labels in both languages. render_page picks the right one
# based on the active language so the labels stay in sync with the
# page content (which is language-specific via pages.py/pages_en.py).
_SECTION_LABELS = {
    "usage":     {"zh_CN": "用法",   "en": "USAGE"},
    "description": {"zh_CN": "描述", "en": "DESCRIPTION"},
    "options":   {"zh_CN": "选项",   "en": "OPTIONS"},
    "examples":  {"zh_CN": "示例",   "en": "EXAMPLES"},
}


def _label(key: str) -> str:
    lang = get_language()
    return _SECTION_LABELS[key].get(lang, _SECTION_LABELS[key]["en"])
...
def aliases_block(aliases) -> None:
    ...
    label = _("Aliases") if get_language() != "zh_CN" else _("别名")
    msg(f"  {C['CYAN']}{label}:{C['RST']} {sep.join(parts)}")
