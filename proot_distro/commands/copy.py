...
def _do_copy(src, dest, verbose, move_mode, recursive):
    ...
    if dest_base in (".", ".."):
        crit_error(_("paths '.' and '..' are not allowed as copy destination."))
        sys.exit(1)

    if not os.path.exists(src_path):
        crit_error(_("cannot copy '{src}' because the path does not exist.", src = src))
        sys.exit(1)

    if not os.access(src_path, os.R_OK):
        crit_error(_("source path '{src_path}' is not readable.", src_path = src_path))
        sys.exit(1)
    ...
    if not os.path.isdir(dest_dir):
        log_info(_("Creating directory '{dest_dir}'...", dest_dir = dest_dir))
        try:
            os.makedirs(dest_dir, exist_ok=True)
        except OSError as exc:
            log_error(_("Cannot create directory '{dest_dir}': {exc}", dest_dir = dest_dir, exc = exc))
            sys.exit(1)

    def _verbose_copy2(src, dst, *, follow_symlinks=True):
        log_info(_("Copying: '{src}' -> '{dst}'", src = src, dst = dst))
        return shutil.copy2(src, dst, follow_symlinks=follow_symlinks)

    try:
        if move_mode:
            log_info(_("Moving files..."))
            if verbose:
                if os.path.isdir(src_path):
                    ...
                    log_info(_("Moving: '{src_path}' -> '{dest_path}'", src_path = src_path, dest_path = dest_path))
            shutil.move(src_path, dest_path)
        else:
            log_info(_("Copying files, this may take a while..."))
            ...
            if os.path.isdir(src_path):
                ...
                if verbose:
                    log_info(_("Copying: '{src_path}' -> '{dest_path}'", src_path = src_path, dest_path = dest_path))
        ...
        log_error(_("Error: {exc}", exc = exc))
        sys.exit(1)

    log_info(_("Finished copying files."))
