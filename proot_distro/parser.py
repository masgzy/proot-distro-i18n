...
def _mirror(sub):
    # 管理内置 Docker registry 镜像源
    p = sub.add_parser(
        "mirror", aliases=["mir", "mr"], add_help=False,
    )
    p._pd_command = "mirror"
    p.add_argument(
        "mirror_action", nargs="?", default="ls",
        metavar="{ls,test,use,unset,show}",
        choices=["ls", "list", "test", "use", "set", "unset", "clear", "off", "show"],
    )
    p.add_argument(
    ...
    p.add_argument(
        "-t", "--timeout", type=float, default=6.0, metavar="SECONDS",
    )
    p.add_argument(
        "--allow-insecure", dest="allow_insecure", action="store_true",
    )
    # `test` action switches:
    use_grp = p.add_mutually_exclusive_group()
    use_grp.add_argument(
        "--use-best", dest="use_best", action="store_true",
        help="after `test`, auto-switch to the fastest reachable mirror (no prompt)",
    )
    use_grp.add_argument(
        "--no-use", dest="no_use", action="store_true",
        help="after `test`, do not prompt or switch",
    )
    # `test` 输出格式:
    p.add_argument(
        "--json", dest="json_output", action="store_true",
        help="`test` output as JSON (script-friendly; implies --no-use)",
    )
    # `ls` 过滤:
    p.add_argument(
        "--reachable-only", dest="reachable_only", action="store_true",
        help="`ls`: show only reachable mirrors (probes them first, may be slow)",
    )
    p.add_argument("-q", "--quiet", action="store_true")
    p.add_argument("-h", "--help", action="store_true")
