...
def get_device_cpu_arch() -> str:
    ...
    raw `uname -m` value. Cached because it never changes during a
    process lifetime.
    """
...
def get_proot_bin() -> str:
    ...
    if override:
        if not os.path.isfile(override) or not os.access(override, os.X_OK):
            crit_error(_("PD_PROOT_BIN '{override}' is not found or not "
                         "executable.", override=override))
            sys.exit(1)
        return override
...
def get_emulator_args(
    ...
    if emulator_override:
        ...
        if not os.path.isfile(emu_path) or not os.access(emu_path, os.X_OK):
            crit_error(_("emulator '{emu_path}' is not found or not executable.", emu_path = emu_path))
            sys.exit(1)
    else:
