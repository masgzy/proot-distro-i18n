...
def container_dir(name: str) -> str:
    ...
    Cached because every subcommand resolves this path multiple times
    (lock + rootfs + manifest + backup), and the join is on a constant
    base — caching saves ~5µs per call on a warm cache.
    """
...
def resolve_container_path(spec: str) -> str:
    ...
    name, _sep, rel_path = spec.partition(":")
    if not is_valid_name(name):
        crit_error(_("invalid container name '{name}' in spec '{spec}'.", name = name, spec = spec))
        sys.exit(1)
    rootfs = os.path.normpath(container_rootfs(name))
    if not os.path.isdir(rootfs):
        crit_error(_("container '{name}' does not exist.", name = name))
        sys.exit(1)
    ...
    if resolved != rootfs and not resolved.startswith(rootfs + os.sep):
        crit_error(_("destination path escapes the container directory."))
        sys.exit(1)
    return resolved
