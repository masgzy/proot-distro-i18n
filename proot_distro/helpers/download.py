...
def insecure_ssl_context() -> ssl.SSLContext:
    ...
    The context is cached on first call (ssl.create_default_context()
    costs ~1ms) — safe because the context is immutable after config.
    """
...
def retry_http(operation, *, what: str, max_retries: int = 5,
    ...
    Robustness: ``max_retries < 1`` is clamped to 1 so the operation is
    always attempted at least once. Previously ``max_retries=0`` would
    silently skip the loop and return None, making callers believe the
    download succeeded.

    Retry delay uses exponential backoff: each retry waits
    ``retry_delay * (2 ** attempt)`` seconds (5s, 10s, 20s, 40s...),
    capped at 60s, so a flaky endpoint doesn't get hammered.
    """
    if max_retries < 1:
        max_retries = 1
...
def download_file(
    ...
    *timeout* (default 60s) is passed to urlopen so a hung connection
    raises URLError instead of blocking forever. Per-chunk reads also
    use this timeout via the socket default.
    """
    ...
    def _attempt():
        ...
        clear_bar()
        log_info(_("Finished downloading ({size}).", size = fmt_size(downloaded)))
