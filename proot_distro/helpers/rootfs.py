...
def register_android_ids(rootfs: str) -> None:
    ...
    # Batch-write all group entries in a single open/close per file,
    # instead of one open/close per group (could be 20+ groups).
    has_gshadow = os.path.exists(gshadow_path)
    group_lines: list[str] = []
    gshadow_lines: list[str] = []
    for g in all_gids:
        try:
            gname = grp.getgrgid(g).gr_name
        except KeyError:
            continue
        group_lines.append(f"aid_{gname}:x:{g}:root,aid_{username_result}\n")
        if has_gshadow:
            gshadow_lines.append(f"aid_{gname}:*::root,aid_{username_result}\n")
    try:
        if group_lines:
            with open(group_path, "a") as fh:
                fh.writelines(group_lines)
        if gshadow_lines:
            with open(gshadow_path, "a") as fh:
                fh.writelines(gshadow_lines)
    except OSError:
        pass
